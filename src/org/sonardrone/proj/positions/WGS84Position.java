 ...
 */
package org.sonardrone.proj.positions;

import java.text.ParseException;

import org.sonardrone.proj.Position;
...
public class WGS84Position extends Position {
	...
	/**
	 * Create a new WGS84 position from a String containing both latitude and
	 * longitude. The string is parsed based on the supplied format.
	 * 
		...
		if (format.equals(WGS84Format.Degrees)) {
			...
			if (lat_lon.length == 2) {
				this.latitude = Double
						.parseDouble(lat_lon[0].replace(",", "."));
				this.longitude = Double.parseDouble(lat_lon[1]
						.replace(",", "."));
			} else {
				throw new ParseException("The position string is invalid", 0);

			}
		} else if (format.equals(WGS84Format.DegreesMinutes)
				|| format.equals(WGS84Format.DegreesMinutesSeconds)) {
			...
			String lat = positionString.substring(0, firstValueEndPos + 1)
					.trim();
	...
	/**
	 * Set the latitude value from a string. The string is parsed based on given
	 * format.
	 * 
	 * @param value
	 * @param format
	...
	/**
	 * Set the longitude value from a string. The string is parsed based on
	 * given format.
	 * 
	 * @param value
	 * @param format
	...
	private String convToDmString(double value, String positiveValue,
		...
		return String.format("%s %.0fº %.0f'", value >= 0 ? positiveValue
				: negativeValue, degrees,
				((double) Math.floor(minutes * 10000) / 10000));
	}

	private String convToDmsString(double value, String positiveValue,
			String negativeValue) {
		...
		return String.format("%s %.0fº %.0f' %.5f\"",
				value >= 0 ? positiveValue : negativeValue, degrees, minutes,
				((double) Math.round(seconds * 100000) / 100000));
	}
